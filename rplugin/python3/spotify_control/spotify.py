...
class Spotify():
    ...
    def make_spotify_request(self, url, method, params, retry_on_fail=True):
        ...
        if method == "POST":
            resp = requests.post(url=url, headers={
                                 "Authorization": "Bearer " + tokens["access_token"]}, data=params)
        elif method == "GET":
            resp = requests.get(url=url, headers={
                                "Authorization": "Bearer " + tokens["access_token"]}, params=params)
        elif method == "PUT":
            resp = requests.put(url=url, headers={
                                "Authorization": "Bearer " + tokens["access_token"], "Content-Type": "application/json"}, data=params)
    ...
    def play_track(self, track_id, context=None):
        ...
        if context:
            ...
            data = {"uris": [track_uri]}
        resp = self.make_spotify_request(
            "https://api.spotify.com/v1/me/player/play", "PUT", json.dumps(data))
        return resp

    def search(self, query):
        data = {'q': query, 'type': 'album,artist,playlist,track'}
        resp = self.make_spotify_request(
            "https://api.spotify.com/v1/search?{}", "GET", data)
        return resp

    def get_artist(self, id):
        url_top_tracks = 'https://api.spotify.com/v1/artists/{}/top-tracks'.format(
            id)
    ...
    def get_search_results(self, search_query):
        ...
        for artist in search_results_data['artists']['items']:
            search_results.append({'title': '  {}'.format(
                artist['name']), 'uri': artist['uri']})
    ...
    def queue_songs(self, songs_data):
        for song_data in songs_data:
            url_queue = 'https://api.spotify.com/v1/me/player/queue?uri={}'.format(
                song_data["uri"])
    ...
    def _parse_tracks_data(self, tracks_data, prefix='', context=None):
        ...
        for data in tracks_data:
            ...
            parsed_track = {'title': '{}{}'.format(
                prefix, title), 'uri': track_data['uri']}
            if context:
    ...
    def _parse_albums_data(self, albums_data, prefix=''):
        ...
        for album in albums_data:
            title = '{}{}  by {}'.format(
                prefix, album['name'], self.get_artists_names(album['artists']))
